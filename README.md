...
- **分栏编辑**：原文 / 分栏 / 仅预览
- **文档目录**：按 Markdown 标题层级生成全局侧栏，可拖拽调宽、收起和点击跳转
- **可编辑预览**：在预览里直接改字，自动同步回 Markdown
- **Mermaid 图表**：离线渲染 `mermaid` 围栏代码块，支持语法错误回退和源码无损回写
...
1. 点工具栏 **Upload to Blog**，或按 `⌘⇧U`（无本地图片时可直接上传；有本地 `assets/` 图片时需先保存文档）
2. 确认标题、日期、标签、摘要、slug（文件名）
3. 上传成功后会提交到：
